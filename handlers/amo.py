...
TASK_TRIGGERS = [
    ...
    "поставьте задачу", "поставьте задание", "поставьте напоминание",
    "задача в crm", "задача в амо", "добавь задачу",
    "запиши задачу", "задачу по", "задачу на",
    "поставь задач", "поставьте задач",
...
def parse_task_datetime(text: str) -> tuple:
    ...
    text = re.sub(r"\b(утра|ночи|вечера|дня|поставьте|поставь|напомни|напомните)\b", "", text, flags=re.IGNORECASE)
    text = re.sub(r"^(по|для|к|на|в)\s+", "", text.strip(), flags=re.IGNORECASE)
