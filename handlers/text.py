...
# Логистические интенты — эти запросы идут в s3 (AI-ассистент), а НЕ в подбор кода
# Даже если в тексте есть товар/страна — менеджер спрашивает про логистику
LOGISTICS_INTENT_KEYWORDS = (
    "какие варианты", "варианты доставки", "как доставить", "как привезти",
    "привезти из", "доставить из", "способы доставки", "маршруты из",
    "сколько везти", "сколько идёт", "сколько дней", "сроки доставки",
    "чем лучше везти", "каким транспортом", "как лучше везти",
    "варианты перевозки", "схема доставки", "логистика из",
    "тонн арматур", "тонн груза", "кубов груза",  # явные логистические запросы
)
...
async def handle_text(message: Message):
    ...
    if not found_codes and not codes:
        # ДЕТЕКЦИЯ ИНТЕНТА
        # Сначала проверяем логистический интент — если менеджер спрашивает
        # про варианты/маршруты/доставку, идём в s3 (AI), а не в подбор кода
        has_logistics_intent = any(kw in text_lower for kw in LOGISTICS_INTENT_KEYWORDS)
        has_ved_intent = (not has_logistics_intent) and any(kw in text_lower for kw in VED_INTENT_KEYWORDS)
    ...
    if is_calc and found_codes:
        ...
        # Получаем курсы для конвертации валют
        try:
            _s3_rates = await get_cbr_rates()
            _usd = _s3_rates.get("USD", "н/д")
            _cny = _s3_rates.get("CNY", "н/д")
            _eur = _s3_rates.get("EUR", "н/д")
            _date = _s3_rates.get("DATE", "")
            _rates_ctx = f"Актуальные курсы ЦБ РФ на {_date}: 1 USD = {_usd} ₽, 1 CNY = {_cny} ₽, 1 EUR = {_eur} ₽"
        except Exception:
            _rates_ctx = ""

        extra = "Ты — внутренний AI-ассистент West Asia для менеджеров по ВЭД. "
        extra += "Все кто пишут тебе — сотрудники West Asia изнутри компании, не внешние клиенты. "
        extra += "Обращайся как к коллеге. НЕ говори 'обратитесь к менеджеру West Asia' — ты помогаешь самому менеджеру. "
        extra += "ДУМАЙ перед ответом: не смешивай разные понятия. "
        extra += "'декларант' = сотрудник West Asia (Анна, Михаил, Александра). 'Агент/экспедитор' = внешний партнёр. "
        if _rates_ctx:
            extra += f"КУРСЫ ВАЛЮТ: {_rates_ctx}. "
        extra += "КОНВЕРТАЦИЯ ВАЛЮТ: если просят перевести сумму в рубли — выполни расчёт по курсу из строки выше. "
        ...
        extra += (
            ...
            "пиши 'уточни точную ставку у декларанта по конкретному коду'. "
