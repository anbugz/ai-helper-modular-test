...
Логика:
  ...
  11. AI-ассистент (общие вопросы) с knowledge_base
...
async def handle_text(message: Message):
    ...
    if is_calc and found_codes:
        ...
        try:
            ...
            if all_knowledge:
                # Лемматизируем слова запроса
                query_words = set()
                for w in re.findall(r'[а-яёa-z]{3,}', text_lower):
                    query_words.add(lemmatize_russian(w))
                    query_words.add(w)
                
                # Ищем совпадения по topic и content
                matched = []
                for k in all_knowledge:
                    ...
                    for qw in query_words:
                        if qw in topic_lower:
                            score += 3  # Совпадение по теме — важнее
