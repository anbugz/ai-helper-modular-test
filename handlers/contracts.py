...
async def extract_company_data(raw_text: str) -> dict:
    prompt = f"""Ты — эксперт по извлечению данных из карточек компаний. Верни ТОЛЬКО JSON без комментариев.
...
НУЖНЫЕ ПОЛЯ:
{{
    "company_name": "Полное название в именительном падеже с кавычками",
    "company_name_short": "Короткое название с кавычками (ООО «...»)",
    "company_name_genitive": "Полное название в родительном падеже",
    "inn": "ИНН 10 или 12 цифр",
    "kpp": "КПП 9 цифр (для ИП — пустая строка)",
    "ogrn": "ОГРН 13 цифр (для ООО)",
    "ogrnip": "ОГРНИП 15 цифр (для ИП)",
    ...
    "ceo_name_genitive": "ФИО руководителя в родительном падеже",
    "ceo_name_genitive_short": "ФИО руководителя в родительном падеже сокращённо",
    "ceo_position_nominative": "Должность. Для ООО: Генеральный директор. Для ИП: Индивидуальный предприниматель",
    "ceo_position_genitive": "Должность в родительном падеже",
    "is_individual_entrepreneur": true или false,
    "legal_basis": "Устава (для ООО) или ОГРНИП (для ИП)",
    "legal_basis_full": "на основании Устава (для ООО) или на основании ОГРНИП (для ИП)",
