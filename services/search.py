...
def _tokenize(text: str) -> List[str]:
    ...
    Короткие транспортные аббревиатуры (жд, жд, sea) обрабатываются отдельно.
    Применяет базовое усечение окончаний для русского языка.
    """
    # Короткие аббревиатуры которые важны
    SHORT_TERMS = {"жд", "жд", "sea", "air"}
    ...
    for t in tokens:
        ...
        for ending in ("ом", "ём", "ем", "ых", "ий", "ая", "ое", "ую", "ой", "ам", "ами", "ах", "ей", "юю", "ью"):
...
def tfidf_search(query: str, top_n: int = 3, min_score: float = 0.05) -> List[Dict]:
    ...
    SYNONYMS = {
        ...
        "морю": ["море", "морской", "sea", "ocean"],   # дательный падеж
        "морем": ["море", "морской", "sea"],            # творительный
        "морск": ["море", "sea"],
