...
def _format_calculation_fallback(
    ...
    if duty_eur_cur and duty > 0:
        ...
        if tariff_type and tariff_type == "min":
    ...
    elif tariff_type == "plus" and duty_eur_cur:
        rows.append((f"📋 Пошлина ({rate:g}% + EUR):", f"{fmt(duty)} {currency}"))
    elif tariff_type == "fixed_eur" and eur_value:
        if duty_eur_cur and weight_kg:
            rows.append((f"📋 Пошлина ({eur_value} EUR/кг × {weight_kg} кг):", f"{fmt(duty)} {currency}"))
        else:
            rows.append(("⚠️ ВНИМАНИЕ:", f"Пошлина {eur_value} EUR/кг"))
            rows.append(("", "❗ УКАЖИТЕ ВЕС для расчёта пошлины"))
            rows.append(("", "   Пример: добавьте 'вес 100 кг' к запросу"))
    else:
